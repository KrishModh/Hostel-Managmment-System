# 🏨 Hostel Management Portal (Student + Rector Panel)

A modern and responsive **Hostel Management Web Application** with: -
Student Portal - Rector/Admin Portal - Authentication System - Leave
Management - Complaint Management - Mess Menu Controls - Dark / Light
Theme - Responsive UI - Toast Notifications

## 🚀 Features

### 👨‍🎓 Student Side

-   Login / Register
-   View Profile & Room Details
-   Apply Leave
-   Track Leave Status
-   File Complaints
-   View Mess Menu

### 🧑‍💼 Rector Side

-   Rector Login
-   Dashboard Overview
-   Approve / Reject Students
-   Assign Rooms & Beds
-   Manage Complaints
-   Approve / Reject Leave
-   Control Mess Menu

### 🎨 UI Features

-   Fully Responsive
-   Dark / Light Theme Toggle
-   Smooth Cards & Glassmorphism
-   Search & Filter
-   Toast Notification instead of alerts

------------------------------------------------------------------------

## 🛠 Tech Stack

**Frontend** - React JS - React Router - react-hot-toast - CSS (custom +
modern UI)

**Backend** - FastAPI - Python - Flask - MongoDB

------------------------------------------------------------------------

## 🖼️ Project Screenshots

_Add screenshots of your project here. You can drag & drop images in GitHub README editor and it will auto-generate links._

### 🔹 Student Portal

#### 🏠 Student Dashboard
<img width="2560" height="1440" alt="Screenshot (607)" src="https://github.com/user-attachments/assets/6b6ccda9-d86e-4fa4-8fc0-272e80ec191d" />

#### 👤 Student Profile
<img width="2560" height="1440" alt="Screenshot (614)" src="https://github.com/user-attachments/assets/db61ab79-f32c-4131-bb57-c3effac928f4" />

#### 📝 Leave Apply Page
<img width="2560" height="1440" alt="Screenshot (612)" src="https://github.com/user-attachments/assets/634b29a1-692f-4856-97f1-9eab62f5e7a8" />

#### 📊 Leave Status Page
<img width="2560" height="1440" alt="Screenshot (613)" src="https://github.com/user-attachments/assets/91cc4712-a63c-4dd5-b1fb-a5194e21b413" />

#### 🛠 Complaint Register Page
<img width="2560" height="1440" alt="Screenshot (608)" src="https://github.com/user-attachments/assets/6a684192-f5f2-43d4-bd59-7fb6833a0e96" />

#### 📬 Complaint Status Page
|<img width="2560" height="1440" alt="Screenshot (609)" src="https://github.com/user-attachments/assets/fe77ae7c-db94-46c2-8697-210a29521df5" />|<img width="2560" height="1440" alt="Screenshot (610)" src="https://github.com/user-attachments/assets/bce9347b-1772-4f01-8650-008f000f8eff" />|<img width="2560" height="1440" alt="Screenshot (611)" src="https://github.com/user-attachments/assets/d7921e53-164b-46fb-8e1b-173f7a2c0393" />|

#### 🍽 Mess Menu View
![Mess Menu](image-link-here)


---

### 🔹 Rector / Admin Portal

#### 🏠 Rector Dashboard
![Rector Dashboard](image-link-here)

#### 🎓 Students Management Page
![Students Page](image-link-here)

#### 📄 Student Full Details Page
![Student Details](image-link-here)

#### 🛏 Room & Bed Allocation Page
![Room Allocation](image-link-here)

#### 🛠 Complaint Management – Pending
![Pending Complaints](image-link-here)

#### 🛠 Complaint Management – Working
![Working Complaints](image-link-here)

#### ✅ Complaint Completed List
![Completed Complaints](image-link-here)

#### 📝 Leave Requests – Pending
![Pending Leave](image-link-here)

#### 🟢 Leave Approved List
![Approved Leave](image-link-here)

#### 🔴 Leave Rejected List
![Rejected Leave](image-link-here)

#### 🍽 Mess Menu Control Panel
![Mess Control](image-link-here)

---

### 🌗 Themes & UI

#### 🌞 Light Mode
![Light Mode](image-link-here)

#### 🌙 Dark Mode
![Dark Mode](image-link-here)

#### 📱 Mobile Responsive View
![Responsive](image-link-here)

------------------------------------------------------------------------

## 📥 Download & Run Project

### ⬇ Direct Download ZIP

After pushing to GitHub your auto download link will be:

    https://github.com/KrishModh/Hostel-Managmment-System/archive/refs/heads/main.zip

Replace `Hostel-Managmment-System` with your repo name.

------------------------------------------------------------------------

## 🧷 Clone Using Git

``` bash
git clone https://github.com/KrishModh/Hostel-Managmment-System.git
cd Hostel-Managmment-System
npm install
npm run dev
```

------------------------------------------------------------------------

## 🔑 Rector Login Credentials

    username: admin
    password: admin123

------------------------------------------------------------------------

## 🤝 Contributing

Pull requests are welcome. For major changes, please open an issue
first.

------------------------------------------------------------------------

## 👤 Author

### Krish Modh

-   🐙 GitHub: https://github.com/KrishModh
-   💼 LinkedIn: https://www.linkedin.com/in/krish-modh-b38447300

------------------------------------------------------------------------

⭐ If you like this project, don't forget to **star the repo**!
